...
{
private:
    typedef std::array<uint8_t, sizeof(net_esp_info_t)> esp_info_buffer;
...
private:
    ...
    void write_to() {
        ...
        // get the current esp_info to send
        m_is_writing = true;
        const net_esp_info_t& net_esp_info = m_write_queue.front();

        // copy packed data to a buffer and send
        std::memcpy(m_write_data.data(), &net_esp_info, sizeof(net_esp_info_t)); 
...
private:
    ...
    esp_info_buffer m_write_data;
    esp_info_buffer m_read_data;
